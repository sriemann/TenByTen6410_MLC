...
int mfcdec_demo(HWND hWnd, LPCTSTR pszFileName[], CODEC_MODE codec_mode[], int num_pips, int *force_exit)
{
    ...
    SSBSIP_H264_STREAM_CROP_INFO stream_crop_info[NUM_PIPS];
...
RETAILMSG(1,(L"\t ------------- \n"));
    ...
    for (pip=0; pip < num_pips; pip++) {
        ...
        if(codec_mode[pip] == CODEC_H264)
        {
            pFP_LIST[pip]->fp_SsbSipDecodeGetConfig(handle[pip], H264_DEC_GETCONF_STREAMINFO_CROP, &(stream_crop_info[pip]));
            RETAILMSG(1,(L"\n\t<STREAMINFO> crop_left=%d   crop_right=%d.\n", stream_crop_info[pip].crop_left, stream_crop_info[pip].crop_right));
            RETAILMSG(1,(L"\n\t<STREAMINFO> crop_top=%d   crop_bottom=%d.\n", stream_crop_info[pip].crop_top, stream_crop_info[pip].crop_bottom));
        }
...
//                *force_exit=1;
        ...
        do
        {
            ret = pFP_LIST[pip]->fp_SsbSipDecodeExe(handle[pip], nFrameLeng[pip]) ;
            ...
            else
            {
                if(ret == -1004)
                {
                    RETAILMSG(1,(L"[APP] MFC Sleeping -> Wait!!! .\r\n"));
                    Sleep(1000);
                }
                else
                    break;
        ...
        if (ret != SSBSIP_MPEG4_DEC_RET_OK)
        {
            RETAILMSG(1,(L"[APP]MPEG4 Decoder Failed! :: ret=%d \r\n", ret));
            sprintf(msg, "\n\t Error in decoding %d-th video, %d-th frame\n", pip, nLoop);
            LogMsg();
//                *force_exit=1;
